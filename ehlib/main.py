        ...
        async def on_page(page: int, items: list[dict], next_cursor: str):
            ...
            for idx, item in enumerate(items, 1):
                ...
                if not sid or sid in metadata_done:
                    continue
                try:
                    artist, thumb_path, uploaded_at, category, cover_url, language = await site.fetch_metadata_and_thumb(sid, thumbs_dir, item.get("thumbnail", ""))
                    await db.update_search_cache_metadata(args.source, sid, artist, thumb_path, uploaded_at, category, cover_url, language)
